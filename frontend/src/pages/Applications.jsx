...
export default function Applications() {
  ...
  return <section><h1>Applications Management</h1><table><thead><tr><th>Job</th><th>Candidate</th><th>Status</th><th>Matching Score</th></tr></thead><tbody>{rows.map(a=>{const m=matchByJob[a.job_id];const label=scoreLabel(m?.overall_score);return <tr key={a.id}><td><b>{a.job_title || a.job_id}</b><br/><span className="muted">{a.company_name}</span></td><td>{a.candidate_name || a.candidate_id}</td><td><span className="badge">{a.status}</span></td><td>{m ? <><b>{Math.round(m.overall_score)}%</b> <span className={label.cls}>{label.text}</span></> : <span className="badge">{label.text}</span>}</td></tr>})}</tbody></table></section>
}
